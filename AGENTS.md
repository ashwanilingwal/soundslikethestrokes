...
| Route | What |
|---|---|
| `/` | The stage: blocking monitor modal, spinning-vinyl transport, pitch readout, 3 macro bars, voice cards, fine tuning, recorder. Below the fold: the ad slot, then the footer |
...
| `/privacy` | Privacy and cookies policy in the guide's formatting, generated from the ads/analytics flags; holds the change-choices control |
...
description, the feature list. `StructuredData` emits a WebApplication
JSON-LD node from it. There was a FAQPage node too, generated from a visible
FAQ below the deck (`SiteCopy`); the owner removed that prose, and structured
data for content the visitor cannot see is what Google's guidelines penalise,
so the FAQ markup went with it. `/guide` is now the page with reading on it.

One structural rule, still load-bearing: **the footer sits OUTSIDE
`<main className="stage">`.** `.stage` is a flex column whose window fills the
screen via `flex: 1`, which only works while there is free space to
distribute; anything tall inside it overflows the container, free space goes
to zero, and the deck silently collapses to its content height - the
one-screen rule breaks with no error.
...
Opt-in regions — EEA, UK, Switzerland, the set in `lib/consentRegion.ts`
(Google's EU-consent-policy set; eval check `v` pins it at 32) — get the
banner and nothing non-essential runs before a yes. Everywhere else the law
is opt-out: tracking runs with no banner and the footer's "cookie choices"
withdraws it. An explicit choice always wins over the regional default.

`src/proxy.ts` reads Vercel's `x-vercel-ip-country` on page requests and
writes a one-day cookie `sslts.region` holding only the verdict (`req` /
`ok`); the client store reads it, and an absent cookie (local dev, another
host) means REQUIRED — asking needlessly costs a click, tracking needlessly
is the offence. The GA consent default is denied for EVERYONE, on purpose
- Consent Mode's `region:` list was tried and rejected, because Google then
resolves the region from the IP itself and, where that differed from our
verdict, granted itself and set cookies under a banner saying nothing runs.
The client pushes the real answer inside the wait window, so ours is the
only verdict that counts.

`/privacy` is the policy page, in the guide's formatting, generated from the
same flags so it never describes tracking that is not configured. It has no
contact address until `PRIVACY_CONTACT` in `lib/site.ts` is filled in.

**AdSense in opt-in regions needs a Google-certified CMP** (TCF v2.2) or it
serves nothing there, however lawful our banner is. `ADSENSE_GOOGLE_MESSAGE`
in `lib/ads.ts` switches to Google's own message (enabled in the AdSense
dashboard): the ads script then always loads, Google's message is the
consent UI in those regions, ours steps aside there.
